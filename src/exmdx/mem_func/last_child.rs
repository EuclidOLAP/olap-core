...
impl AstMemberFnLastChild {
    pub async fn do_get_member(
        ...
        let member_role: Option<MemberRole> = match self {
            AstMemberFnLastChild::MemberSegs(member_segs) => match member_segs
        ...
        if member_role.is_none() {
            panic!("[lc-001] LastChild requires a member (MemberSegs) or outer_param that resolves to a MemberRole");
        }

        let member_role = member_role.unwrap();

        // TODO: implement actual last-child logic.
        match member_role {
            ...
            MemberRole::FormulaMember { .. } => {
                panic!("[lc-002] LastChild not supported for FormulaMember");
            }
        }
    }
}
