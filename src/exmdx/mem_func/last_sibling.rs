...
impl AstMemberFnLastSibling {
    pub async fn do_get_member(
        ...
        let member_role: Option<MemberRole> = match self {
            AstMemberFnLastSibling::MemberSegs(member_segs) => match member_segs
        ...
        if member_role.is_none() {
            panic!("[ls-001] LastSibling requires a member (MemberSegs) or outer_param that resolves to a MemberRole");
        }

        let member_role = member_role.unwrap();

        // TODO: implement actual last-sibling logic.
        match member_role {
            ...
            MemberRole::FormulaMember { .. } => {
                panic!("[ls-002] LastSibling not supported for FormulaMember");
            }
        }
    }
}
